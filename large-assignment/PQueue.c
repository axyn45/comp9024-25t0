// Priority Queue ADO implementation ... COMP9024 25T0
...
void PQueueInit() { PQueue.length = 0; }
...
void joinPQueue(int v) {
    ...
    while (i < PQueue.length &&
           PQueue.item[i] != v)  // check if v already in queue
        i++;
...
Vertex leavePQueue(int priority[]) {
    ...
    PQueue.length--;
    PQueue.item[bestIndex] =
        PQueue.item[PQueue.length];  // replace dequeued node
                                     // by last element in array
    return bestVertex;
}

// check if priority queue PQueue is empty
bool PQueueIsEmpty() { return (PQueue.length == 0); }